...
export class AudioEngine {
  ...
  public getContext(): AudioContext {
    if (!this.ctx) {
      ...
      this.lofiFilter.frequency.value = 20000; // Default off

      // Echo Setup (Simple mono delay with feedback)
      ...
      // Signal Chain: Tone/Noise -> Fade -> Distortion -> Reverb/Echo Branches -> LoFi Filter -> 8D Panner -> Master -> Analyzer -> Output
      this.fadeGain.connect(this.distortionNode);

      // Echo/Reverb parallel branches
  ...
  private generateBuffers() {
    ...
    for (let i = 0; i < length; i++) {
      // Background hiss (crackle)
      let sample = (Math.random() * 2 - 1) * (0.002 + this.crackleIntensity * 0.01);
      // Random pops (controlled by popIntensity)
  ...
  private start8DAnimation() {
    const loop = () => {
      if (this.ctx && this.panner) {
        // Vary speed based on analyzer data
        let speedFactor = 0.005;
        if (this.analyzer) {
          const data = new Uint8Array(this.analyzer.frequencyBinCount);
          this.analyzer.getByteFrequencyData(data);
          const average = data.reduce((a: number, b: number) => a + b) / data.length;
          speedFactor = 0.002 + (average / 255) * 0.015; // Range: 0.002 to 0.017
        }

        this.eightDTick += speedFactor;
        // Only apply if context is running and active
      }
  ...
  private generateReverbIR() {
    ...
    const length = this.ctx.sampleRate * 2.5; // 2.5 seconds
  ...
  public setEchoVolume(vol: number) {
    if (this.echoGain && this.ctx) {
      this.echoGain.gain.setTargetAtTime(vol * 0.6, this.ctx.currentTime, 0.1);
    }
  }

  public setReverbVolume(vol: number) {
    if (this.reverbGain && this.ctx) {
      this.reverbGain.gain.setTargetAtTime(vol * 0.5, this.ctx.currentTime, 0.1);
    }
  }

  public setDistortionAmount(amount: number) {
    if (this.distortionNode && this.ctx) {
  ...
  public setLofiPopIntensity(intensity: number) {
    ...
    if (this.ctx && this.vinylSource) {
      // Regenerate buffers with new intensity if active, or just adjust gain?
      // Better: Buffer generation should be independent of intensity if possible, 
      // but here it's baked in. I'll just regenerate buffers for simplicity.
      this.generateBuffers();
  ...
  public set8DActive(active: boolean) {
    if (!this.ctx || !this.panner) return;
    if (active) {
      const panUpdate = () => {
        if (!active) return;
        // The tick is already updated in start8DAnimation, we just use it here
  ...
  public setFadeLevel(level: number) {
    if (this.fadeGain && this.ctx) {
      this.fadeGain.gain.setTargetAtTime(level, this.ctx.currentTime, 0.1);
    }
  }
  ...
  public setToneVolume(vol: number) {
    if (this.toneGain && this.ctx) {
      this.toneGain.gain.setTargetAtTime(vol * 0.4, this.ctx.currentTime, 0.1);
    }
  }
  ...
  // Added resume for compatibility
  public resume() {
